...
import LandingPage from './components/LandingPage'; // استدعاء الصفحة الجديدة

declare global { interface Window { paypal: any; } }

// تمت إضافة 'landing' إلى الحالات الممكنة للتطبيق
type AppState = 'landing' | 'auth' | 'subscription' | 'leagues' | 'matches' | 'analysis';
interface UserSession { email: string; subscribed?: boolean; }

const App: React.FC = () => {
  // الحالة الأولية أصبحت 'landing' بدل 'auth'
  ...
  useEffect(() => {
    ...
    const initSession = async () => {
      try {
        ...
        if (session && isMounted) {
            // إذا كان المستخدم مسجلاً للدخول بالفعل، نتجاوز الـ Landing والـ Auth
            handleUserRouting(session.user.email || '');
        }
      } catch (err) {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => {
      if (isMounted) {
        ...
        else { setUser(null); } // عند الخروج نبقى في الحالة الحالية أو نعود للبداية يدوياً
      }
    });
    return () => {
      isMounted = false;
  ...
  const handleLogout = async () => { 
    try {
      ...
      setUser(null); 
      setAppState('landing'); // العودة لصفحة الهبوط عند تسجيل الخروج
      paypalRenderingRef.current = false;
      if (paypalButtonInstance.current && paypalButtonInstance.current.close) {
        paypalButtonInstance.current.close().catch(() => {});
      }
